...
function CanvasMediaModal() {
  ...
  return (
    <div className="p-2 h-screen">
      <button
        className="text-sm p-1 w-full rounded bg-violet-700 hover:bg-violet-500 text-white"
        onClick={open}
      >
        画像をアップロード
      </button>
              ...
              images.map((image, i) => (
                <div key={i} className="group relative">
                  <button
                    ...
                    className="absolute invisible right-1 top-1 bg-red-500 text-white rounded px-2 group-hover:visible hover:bg-red-600 hover:text-gray-100"
                  >
                    ×
                  </button>
                  <img
                    src={image.src}
                    loading="lazy"
                    onDragStart={(event) => handleCreateElement(event, image)}
                  />
                </div>
              ))}
          </div>
